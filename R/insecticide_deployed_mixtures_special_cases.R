...
insecticide_deployed_mixtures_special_cases = function(exposure.scaling.factor = 10,
                                                       nsim = 1000,
  ...
  direct.response.decay = response_to_insecticide_selection_decay(exposure.scaling.factor = exposure.scaling.factor,
                                                                  nsim = nsim,
  ...
  direct.costs = effect_of_fitness_cost(resistance.cost = resistance.cost,
  ...
  indirect.response.decay =  insecticide_deployed_cross_selection_decay(currently.deployed.insecticide = currently.deployed.insecticide,
  ...
  field.survival = convert_bioassay_survival_to_field_insecticide_decay(bioassay.survival = survival.to.other.insecticide,
                                                      conversion.factor = conversion.factor,
                                                      intercept = intercept,
                                                      current.insecticide.efficacy = efficacy.of.other.insecticide)



  track.resistance.intensity = initial.resistance.intensity + ((direct.response.decay +
                                                                  direct.costs +
                                                                  indirect.response.decay)*
                                                                 field.survival)
