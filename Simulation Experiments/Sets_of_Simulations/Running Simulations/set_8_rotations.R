...
for(v in 1:nrow(parameter.space)){
  ...
  start.score = start.resistance[v]
...
rotation.df.set8 = do.call(rbind, temp.list.rotation)
rotation.df.set8.complete = cbind(rotation.df.set8, parameter.space)

write.csv(rotation.df.set8.complete, ".//rotation.set8.csv")
