#Set 9: sequence, Cross Resistance, 50 starting resistance. Return threshold 9%
...
temp.list.sequence.cross.set.9 = list()
for(v in 1: nrow(parameter.space.df)){
  ...
  temp.list.sequence.cross.set.9[[v]] = temp_2
}

sequence.df.cross = do.call(rbind, temp.list.sequence.cross.set.9)
sequence.df.set.9 = cbind(sequence.df.cross, parameter.space.df)

write.csv(sequence.df.set.9, ".//sequence.set.9.csv")
