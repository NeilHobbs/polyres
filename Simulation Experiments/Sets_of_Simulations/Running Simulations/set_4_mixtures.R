...
for(v in 1:nrow(parameter.space.df)){
  ...
  temp.list.mixtures.set.4[[v]] = temp_2
}

mixture.df.set.4 = do.call(rbind, temp.list.mixtures.set.4)
mixture.df.set.4.1 = cbind(mixture.df.set.4, parameter.space.df)

write.csv(mixture.df.set.4.1, ".//mixtures.set.4.csv")
