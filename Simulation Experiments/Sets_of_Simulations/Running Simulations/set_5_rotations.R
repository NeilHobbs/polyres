#Set 5: rotation, Cross Resistance, 50 starting resistance.
...
temp.list.rotation.cross.set.5 = list()
for(v in 1: nrow(parameter.space.df)){
  ...
  temp.list.rotation.cross.set.5[[v]] = temp_2
}

rotation.df.cross = do.call(rbind, temp.list.rotation.cross.set.5)
rotation.df.set.5 = cbind(rotation.df.cross, parameter.space.df)

write.csv(rotation.df.set.5, ".//rotation.set.5.csv")
