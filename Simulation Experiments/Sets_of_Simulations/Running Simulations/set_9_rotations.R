#Set 9: rotation, Cross Resistance, 50 starting resistance.

#read in previously used random parameters
parameter.space = read.csv("Simulation Experiments/Sets_of_Simulations/Setting up Simulations/parameter.space.2.csv")
parameter.space = parameter.space[,2:7] #only need the first 5000 rows
...
temp.list.rotation.cross.set.9 = list()
for(v in 1: nrow(parameter.space.df)){
  ...
  temp.list.rotation.cross.set.9[[v]] = temp_2
}

rotation.df.cross = do.call(rbind, temp.list.rotation.cross.set.9)
rotation.df.set.9 = cbind(rotation.df.cross, parameter.space.df)

write.csv(rotation.df.set.9, ".//rotation.set.9.csv")
