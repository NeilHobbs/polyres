#Test 4:
#Uninheritable resistance:
#
#The Model is working correctly if:
#1. Resistance is not inheritable, so should remain 0.
#2. As there is no dispersal, refugia should see no change in resistance intensity
#3. Resistance does not fall below 0.

Test_4 = run_simulation (number.of.insecticides = 3,
                         exposure.scaling.factor = 10,
                         nsim = 1000, #default
                         minimum.insecticide.resistance.hertitability = 0, #default 
                         maximum.insecticide.resistance.hertitability = 0, #default
...
refugia1 = Test_4["refugia", 1, ] #Refugia, insecticide 1 length 500
refugia2 = Test_4["refugia", 2, ] # Refugia, insecticide 2 length 500
refugia3 = Test_4["refugia", 3, ] #Refugia, insecticide 3 length 500

#Do the same, but for treatment sites
treatment1 = Test_4["treatment", 1, ] #treatment, insecticide 1 length 500
treatment2 = Test_4["treatment", 2, ] # treatment, insecticide 2 length 500
treatment3 = Test_4["treatment", 3, ] #treatment, insecticide 3 length 500
...
Test4_DF = data.frame(generation, location, resistance.intensity, resistance.insecticide, deployed)
...
ggplot(Test4_DF, aes(x=generation, y = resistance.intensity)) +
  geom_point(aes(colour = resistance.insecticide)) +
  geom_point(aes(y=-0.1, colour = deployed, x = generation)) +
   theme_classic() +
  labs(colour = "Insecticide")+
  ylab("Insecticide Resistance Intensity") +
  xlab("Mosquito Generation") +
  ggtitle("Uninheritable Resistance") +
