...
test_that("min heritability in range",{
  expect_error(insecticide_not_deployed_migration(
    ...
    max.intervention.coverage = 0.9, 
    min.dispersal.rate = 0.1,
    max.dispersal.rate = 0.9), "minimum.insecticide.resistance.hertitability must be between 0 and 1")})}
...
for(i in 1:length(test.values)){
  test_that("max heritability in range",{
    expect_error(insecticide_not_deployed_migration(
      ...
      max.intervention.coverage = 0.9, 
      min.dispersal.rate = 0.1,
      max.dispersal.rate = 0.9), "maximum.insecticide.resistance.hertitability must be between 0 and 1")})}
...
test_that("heritability min > max",{
    expect_error(insecticide_not_deployed_migration(
      ...
      max.intervention.coverage = 0.9, 
      min.dispersal.rate = 0.1,
      max.dispersal.rate = 0.9), "minimum.insecticide.resistance.hertitability is greater than maximum.insecticide.resistance.hertitability")})
